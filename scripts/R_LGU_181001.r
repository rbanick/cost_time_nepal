...
lgu_input_slim <- subset(lgu_input, select = c("adm_name","adm_code","trav_value","trav_cat","adm_pop","cat_pop","pc_pop")) # create working subset
lgu_input_slim$trav_cat <- factor(lgu_input_slim$trav_cat, levels = c("0 to 1 hours","1 to 2 hours","2 to 4 hours","4 to 8 hours","8 to 16 hours","16 to 24 hours","24 to 36 hours","> 36 hours"))  #order trav_cat in the right way
...
lgu_input_slim[order(lgu_input_slim$adm_code,lgu_input_slim$trav_cat),]

## define color palette variableslgu_list=list()

LtoM <-colorRampPalette(c('yellow', 'red' ))
Mid <- "snow3"
MtoH <-colorRampPalette(c('darkgreen', 'lightgreen'))
cols <- c("1" = 'darkgreen', "2" = '#2ab72e', "3" = 'lightgreen', "4" = '#f4f007', "5" = '#FECC42', "6" = '#f96a34', "7" = "red")
...
for (i in unique(lgu_input_slim$adm_code)) {

  lgu_filter <- filter(lgu_input_slim, adm_code == i)
  ...
  filename=paste(lgu_filter$adm_name," - ",i,".pdf",sep="")
...
for (i in unique(lgu_input_slim$adm_code)) { # Another for loop, this time to save out the bar charts in lgu_list as PDFs
    lgu_filter <- filter(lgu_input_slim, adm_code == i)
    filename=paste(lgu_filter$adm_name," - ",i,".jpeg",sep="") # Make the file name for each PDF. The paste makes the name a variable of the admin, so each chart is named by LGU and code
