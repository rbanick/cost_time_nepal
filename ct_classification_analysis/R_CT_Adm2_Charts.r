...
for (i in unique(lgu_input_slim$adm_code)) {
  ...
  filename=paste(lgu_filter$adm_name," - ",i," - ",type,".pdf",sep="")
...
for (i in unique(lgu_input_slim$adm_code)) { # Another for loop, this time to save out the bar charts in lgu_list as PDFs
    lgu_filter <- filter(lgu_input_slim, adm_code == i)
    filename=paste(lgu_filter$adm_name," - ",i," - ",type,".jpeg",sep="") # Make the file name for each PDF. The paste makes the name a variable of the admin, so each chart is named by LGU and code
