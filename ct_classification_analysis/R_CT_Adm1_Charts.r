...
for (i in unique(adm1_input_slim$adm_code)) {

  adm1_filter <- filter(adm1_input_slim, adm_code == i)
  ...
  filename=paste(adm1_filter$adm_name," - ",i," - ",type,".pdf",sep="")
...
for (i in unique(adm1_input_slim$adm_code)) { # Another for loop, this time to save out the bar charts in adm1_list as PDFs
    adm1_filter <- filter(adm1_input_slim, adm_code == i)
    filename=paste(adm1_filter$adm_name," - ",i," - ",type,".jpeg",sep="") # Make the file name for each PDF. The paste makes the name a variable of the admin, so each chart is named by LGU and code
